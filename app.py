...
def render_article_block(title, table_df, chart_title,
    ...
    st.plotly_chart(fig, use_container_width=True)
        # ── Слайсер-подібний список магазинів під графіком ─────────────────────
    st.markdown("##### 🏪 Магазини по статті (сумарно за період)")
    
    if df_filtered is not None and not df_filtered.empty:
        sub = df_filtered[
            (df_filtered[col_article] == title) &
            (df_filtered[col_plf].isin(["F", "PL"]))
        ].copy()
        
        if not sub.empty:
            sub[col_value] = pd.to_numeric(sub[col_value], errors="coerce")
            
            # Агрегуємо по магазинах
            tt_summary = (
                sub.groupby([col_tt, col_plf])[col_value]
                .sum()
                .unstack(fill_value=0)
            )
            
            # Якщо немає PL або F — додаємо колонку з нулями
            if "F" not in tt_summary.columns:
                tt_summary["F"] = 0
            if "PL" not in tt_summary.columns:
                tt_summary["PL"] = 0
                
            tt_summary = tt_summary.rename(columns={"F": "Fact", "PL": "Plan"})
            tt_summary["Delta"] = tt_summary["Fact"] - tt_summary["Plan"]
            tt_summary["% до плану"] = (
                (tt_summary["Fact"] / tt_summary["Plan"] - 1) * 100
            ).where(tt_summary["Plan"] != 0)
            
            tt_summary = tt_summary.reset_index()
            tt_summary = tt_summary.sort_values("Delta")  # від найкращих до гірших
            ...
            styled_summary = tt_summary.style.format({
                "Fact": fmt,
                "Plan": fmt,
                "Delta": lambda x: f"{x:+,.0f}".replace(",", " ") if pd.notna(x) else "—",
                "% до плану": lambda x: f"{x:+.1f}%" if pd.notna(x) else "—"
            }).background_gradient(
                cmap="RdYlGn_r", subset=["Delta"], vmin=-tt_summary["Delta"].abs().max(),
                vmax=tt_summary["Delta"].abs().max()
            ).background_gradient(
                cmap="RdYlGn", subset=["% до плану"]
            )
            ...
            st.dataframe(
                ...
                hide_index=True,
                height=min(400, len(tt_summary) * 35 + 40)
            )
            
            # Кнопка для скачування тільки цього слайсера
            csv = tt_summary.to_csv(index=False, encoding="utf-8-sig")
            st.download_button(
                label="⬇️ Завантажити список магазинів (CSV)",
                data=csv,
                file_name=f"Магазини_{title}.csv",
                mime="text/csv",
                key=f"download_tt_{title}"
            )
        else:
